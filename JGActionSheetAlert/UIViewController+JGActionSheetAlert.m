//
//  UIViewController+JGActionSheetAlert.m
...
#import "JGSourcePrivateDefine.h"

@class JGActionSheetAlert;

@implementation UIAlertController (JGActionSheetAlert)
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (NSString *oriSelName in oriSelectors) {
            ...
            SEL swizzledSelector = NSSelectorFromString([@"JGActionSheetAlert_" stringByAppendingString:oriSelName]);
...
- (void)JGActionSheetAlert_dealloc {
    
    JGLogFunc(@"<%@: %p>", NSStringFromClass([self class]), self);
    
    [self JGActionSheetAlert_dealloc];
}
...
@implementation UIViewController (JGActionSheetAlert)
